...
class Wide_ResNet(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        linearLayers = [
            m for m in self.modules() if isinstance(m, nn.modules.linear.Linear)
        ]
        convlayers = [
            m for m in self.modules() if isinstance(m, (nn.Conv2d, e2nn.R2Conv))
        ]
        weights_conv = [
            p
            for m in convlayers
            for n, p in m.named_parameters()
            if n.endswith("weights") or n.endswith("weight")
        ]
        biases = [p for n, p in self.named_parameters() if n.endswith("bias")]
        weights_bn = [
            p
            for m in batchnormLayers
            for n, p in m.named_parameters()
            if n.endswith("weight") or n.split(".")[-1].startswith("weight")
        ]
        weights_fully = [
            p
            for m in linearLayers
            for n, p in m.named_parameters()
        ...
        for n, p in self.named_parameters():
            if p.requires_grad and not n.endswith(("weight", "weights", "bias")):
                raise Exception(
                    "named parameter encountered which is neither a weight nor a bias but `{:s}`".format(
                        n
                    )
                )
        param_groups = [
            ...
            dict(
                params=weights_fully,
                lamb_L1=1e-8,
                lamb_L2=1e-8,
                weight_decay=1e-8,
            ),
            dict(
                params=weights_softmax,
                lamb_L1=0,
                lamb_L2=0,
                weight_decay=0,
            ),
        ...
        return torch.optim.Adam(param_groups, lr=0.1, betas=(0.9, 0.999))
